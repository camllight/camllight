...
(* Not exported , to share same interface with std io.ml *)
let dump_counters () =
  begin 
    try 
      let ic = open_in_bin "camlpro.dump" in
      let prevl = input_value ic in
      	close_in ic;
        ...
        do_list2 (fun (curname, (_,curcount)) (prevname, (_,prevcount))
                   -> for i = 0 to vect_length curcount - 1 do
                        curcount.(i) <- curcount.(i) + prevcount.(i)
                      done)
                  !profiler__counters prevl
    with _ -> ()
  end;
  let c = open_out_bin "camlpro.dump" in
    output_value c !profiler__counters;
    close_out c
;;
