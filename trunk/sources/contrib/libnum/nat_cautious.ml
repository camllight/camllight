...
let badsubnat name nat off len = 
    sprintf "%s: Bad subnat, (offset = %d) + (length = %d) \
               >= (length (nat) = %d)"
             name off len (nat__length_nat nat);;
...
let check_subnat nat off len name =
 ...
 if gt_int (add_int off len) (nat__length_nat nat) then
...
let check_offset nat off name =
 if lt_int off 0 then failwith (badoff_inf name off) else 
 let len = nat__length_nat nat in
...
let baddigit name nat off = 
    ...
    name off (nat__sys_string_of_nat 10 "" nat off 1 "");;

let check_digit nat off name =
  check_offset nat off name;
  if not (nat__is_digit_int nat off) then
...
let set_to_zero_nat nat off len =
 ...
 nat__set_to_zero_nat nat off len;;
...
let incr_nat nat off len carry =
 ...
 nat__incr_nat nat off len carry;;
...
let div_digit_nat nat1 off1 nat2 off2 nat3 off3 len nat4 off4 =
 ...
 if le_int (nat__length_nat nat1) (add_int off1 len)
  then failwith (sprintf 
   "div_digit_nat: the quotient size = %d must be greater than or equal to %d" 
   (nat__length_nat nat1) (add_int off1 len)) else
...
let length_nat = nat__length_nat;;
...
let sys_string_of_nat base before nat off len after = 
  ...
  nat__sys_string_of_nat base before nat off len after;;
...
let sys_string_list_of_nat base nat off len = 
  ...
  nat__sys_string_list_of_nat base nat off len;;
...
let sys_print_nat base before nat off len after =
  ...
  nat__sys_print_nat base before nat off len after;;
...
let sys_float_of_nat nat off len = 
  ...
  nat__sys_float_of_nat nat off len;;
...
let gcd_nat nat1 off1 len1 nat2 off2 len2 = 
  ...
  nat__gcd_nat nat1 off1 len1 nat2 off2 len2;;
...
let le_nat nat1 off1 len1 nat2 off2 len2 = 
  ...
  nat__le_nat nat1 off1 len1 nat2 off2 len2;;
...
let lt_nat nat1 off1 len1 nat2 off2 len2 = 
  ...
  nat__lt_nat nat1 off1 len1 nat2 off2 len2;;
...
let ge_nat nat1 off1 len1 nat2 off2 len2 = 
  ...
  nat__ge_nat nat1 off1 len1 nat2 off2 len2;;
...
let gt_nat nat1 off1 len1 nat2 off2 len2 = 
  ...
  nat__gt_nat nat1 off1 len1 nat2 off2 len2;;
