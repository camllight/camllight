...
let badcarry name carry = 
    sprintf "%s: Bad carry (%s) must be 0 or 1" name (string_of_int carry);;

let badborrow name borrow = 
    sprintf "%s: Bad borrow (%s) must be 0 or 1" name (string_of_int borrow);;

let badresplen name len1 len2 = 
    sprintf "%s: Second argument (len2 = %s) \
            longer than the first one (len1 = %s)"
            name (string_of_int len2) (string_of_int len1);;

let badrespleneq name len1 = 
    sprintf "%s: subnat arguments having same length (%s) \
             and the result does not fit in the first subnat"
            name (string_of_int len1);;

let badoff name off = 
    sprintf "%s: Bad offset (%s) must be positive or null"
            name (string_of_int off);;

let badlen name len = 
    sprintf "%s: Bad length (%s)) must be greater than or equal to 1"
            name (string_of_int len);;

let badlen0 name len = 
    sprintf "%s: Bad length (%s) must be positive or null"
            name (string_of_int len);;

let badsubnat name nat off len = 
    sprintf "%s: Bad subnat, (offset = %s) + (length = %s) \
               >= (length (nat) = %s)"
             name (string_of_int off) (string_of_int len)
             (string_of_int (length_nat nat));;

let baddigit name nat off = 
    sprintf "%s: Bad digit, (offset = %s) >= (length (nat) = %s)"
            name (string_of_int off) (string_of_int (length_nat nat));;

let badoff1 name off1 = 
    sprintf "%s: Bad offset (off1 = %s) \
             for first nat argument, must be positive or null"
            name (string_of_int off1);;

let badlen1 name len1 = 
    sprintf "%s: Bad length (len1 = %s) \
...
let badsubnat1 name nat1 off1 len1 = 
    sprintf "%s: Bad subnat for first nat argument, \
             (offset1 = %s) + (length1 = %s) \
             >= (length (nat1) = %s)" 
             name (string_of_int off1) (string_of_int len1) 
             (string_of_int (length_nat nat1));;

let baddigit1 name nat1 off1 = 
    sprintf "%s: Bad digit for first nat argument, (offset1 = %s) \
              >= (length (nat1) = %s)"
             name (string_of_int off1) (string_of_int (length_nat nat1));;

let badoff2 name off2 =
    sprintf "%s: Bad offset (off2 = %s) \
             for second nat argument must be positive or null"
            name (string_of_int off2);;

let badlen2 name len2 = 
    sprintf "%s: Bad length (len2 = %s) \
             for second nat argument must be greater than or equal to 1"
            name (string_of_int len2);;
...
let badsubnat2 name nat2 off2 len2 = 
    sprintf "%s: Bad subnat for second nat argument, (offset2 = %s) \
             + (length2 = %s) >= (length (nat2) = %s)"
            name (string_of_int off2) (string_of_int len2)
            (string_of_int (length_nat nat2));;

let baddigit2 name nat2 off2 = 
    sprintf "%s: Bad digit for second nat argument, (offset2 = %s) \
             >= length (nat2) = %s"
            name (string_of_int off2) (string_of_int (length_nat nat2));;

let badoff3 name off3 = 
    sprintf "%s: Bad offset (off3 = %s) \
            for third nat argument, must be positive or null"
            name (string_of_int off3);;

let badlen3 name len3 = 
    sprintf "%s: Bad length (len3 = %s) \
             for third nat argument, must be greater than or equal to 1"
            name (string_of_int len3);;

let badsubnat3 name nat3 off3 len3 =
    sprintf "%s: Bad subnat for second nat argument, (offset3 = %s) \ 
             + (length3 = %s) >= (length (nat3) = %s)"
            name (string_of_int off3) (string_of_int len3) 
            (string_of_int (length_nat nat3));;

let baddigit3 name nat3 off3 = 
    sprintf "%s: Bad digit for second nat argument, (offset3 = %s) \ 
             >= length (nat3) = %s"
            name (string_of_int off3) (string_of_int (length_nat nat3));;

let badoff4 name off4 = 
    sprintf "%s: Bad offset (off4 = %s) \
             for fourth nat argument, must be positive or null"
            name (string_of_int off4);;

let max_nat_size = (8 * sys__max_string_length) / length_of_digit;;
...
(*
(synonym 'BnNumDigits_old 'BnNumDigits)
(synonym 'BnSetDigit_old 'BnSetDigit)
(synonym 'BnAssign_old 'BnAssign)
(synonym 'BnSetToZero_old 'BnSetToZero)
(synonym 'BnGetDigit_old 'BnGetDigit)
(synonym 'IsDigitInt_old 'IsDigitInt)
(synonym 'IsNatInt_old 'IsNatInt)
(synonym 'IsDigitNormalized_old 'IsDigitNormalized)
(synonym 'IsDigitZero_old 'IsDigitZero)
(synonym 'IsDigitOdd_old 'IsDigitOdd)
(synonym 'BnNumLeadingZeroBitsInDigit_old 'BnNumLeadingZeroBitsInDigit)
(synonym 'BnAddCarry_old 'BnAddCarry)
(synonym 'BnAdd_old 'BnAdd)
(synonym 'BnComplement_old 'BnComplement)
(synonym 'BnSubtractBorrow_old 'BnSubtractBorrow)
(synonym 'BnSubtract_old 'BnSubtract)
(synonym 'BnMultiplyDigit_old 'BnMultiplyDigit)
(synonym 'BnMultiply_old 'BnMultiply)
(synonym 'BnShiftLeft_old 'BnShiftLeft)
(synonym 'BnDivide_old 'BnDivide)
(synonym 'BnDivideDigit_old 'BnDivideDigit)
(synonym 'BnShiftRight_old 'BnShiftRight)
(synonym 'BnCompareDigits_old 'BnCompareDigits)
(synonym 'BnCompare_old 'BnCompare)
(synonym 'BnAndDigits_old 'BnAndDigits)
(synonym 'BnOrDigits_old 'BnOrDigits)
(synonym 'BnXorDigits_old 'BnXorDigits)*)
...
let set_digit_nat nat off digit =
 ...
 if ge_int off (length_nat nat)
  then failwith
    (sprintf "set_digit_nat: Bad offset (%s) \
              must be lesser than length (nat) (%s)"
              (string_of_int off)
              (string_of_int (length_nat nat)))
...
let blit_nat nat1 off1 nat2 off2 len =
 ...
 if gt_int (add_int off2 len) (length_nat nat2) then
  (failwith
   (sprintf "blit_nat: Bad second subnat, (offset2 = %s) \
             + (length = %s) >= (length (nat2) = %s)"
            (string_of_int off2) (string_of_int len) 
            (string_of_int (length_nat nat2)))) else
 if gt_int (add_int off1 len) (length_nat nat1) then
  (failwith
   (sprintf "blit_nat: Bad first subnat, (offset1 = %s) \
             + (length = %s) >= (length (nat1) = %s)"
            (string_of_int off1) (string_of_int len)
            (string_of_int (length_nat nat1)))) else
...
let nth_digit_nat nat off =
 ...
 if not (nat__is_digit_int nat off)
  then failwith
   (sprintf "nth_digit_nat: too many bits for an int, digit = %s"
            (string_of_int (nat__nth_digit_nat nat off))) else
     ...
     failwith 
      (sprintf
         "add_nat: First and second nat arguments are the same, \
          so offset (off2 = %s) should be greater than (off1 = %s), \
          or subnat (nat2, off2, len2) should not overlap subnat \
          (nat1, off1, len1) but off2+len2 = %s > off1 = %s"
          (string_of_int off2)
          (string_of_int off1)
          (string_of_int (add_int off2 len2)) (string_of_int off1))
...
let sub_nat nat1 off1 len1 nat2 off2 len2 borrow =
 ...
 if nat1 == nat2 && gt_int off1 off2
  then failwith
   (sprintf 
     "sub_nat: First and second nat are the same so offset (off2 = %s) \
     should be greater than (off1 = %s)"
     (string_of_int off2)
     (string_of_int off1))
  ...
  then failwith
   (sprintf 
    "mult_nat: The length of the first argument \
     ...
     len1 = %s, len2 = %s, len3 = %s"
     (string_of_int len1) (string_of_int len2) (string_of_int len3)) else
 if nat1 == nat2 &&
...
let shift_left_nat nat1 off1 len1 nat2 off2 shift =
 ...
 if lt_int shift 0 || gt_int shift sys__word_size
  then failwith (sprintf 
   "shift_left_nat: shift number out of range, shift = %s" 
   (string_of_int shift))
...
let div_nat nat1 off1 len1 nat2 off2 len2 =
 ...
 if le_int len1 len2
  then failwith
   (sprintf 
     "div_nat: length of the dividend = %s \
      must be greater than the divisor one's = %s"
     (string_of_int len1) (string_of_int len2)) else
 if le_int
...
let div_digit_nat nat1 off1 nat2 off2 nat3 off3 len nat4 off4 =
 ...
 if lt_int (length_nat nat1) (pred (add_int off1 len))
  then failwith (sprintf 
   "div_digit_nat: the quotient size = %s must be greater than or equal to %s" 
   (string_of_int (length_nat nat1)) (string_of_int (add_int off1 len))) else
 if lt_int off2 0 then failwith (badoff2 "div_digit_nat" off2) else
 if ge_int off2 (length_nat nat2)
  then failwith (baddigit2 "div_digit_nat" nat2 off2) else
 if le_int len 1 
  then failwith (sprintf 
   "div_digit_nat: Bad length must be greater than 1, len = %s" 
   (string_of_int len)) else
 if lt_int off3 0 then failwith (badoff3 "div_digit_nat" off3) else
 if gt_int (add_int off3 len) (length_nat nat3)
  then failwith (sprintf 
   "div_digit_nat: Bad third subnat, (off3 = %s) \
    + (len = %s) >= length (nat3) = %s)" 
   (string_of_int off3) (string_of_int len) 
   (string_of_int (length_nat nat3))) else
...
let shift_rignt_nat nat1 off1 len1 nat2 off2 shift =
 ...
 if lt_int shift 0 || gt_int shift nat__length_of_digit
  then failwith (sprintf 
   "shift_right_nat: shift value out of range, shift = %s" 
   (string_of_int shift))
