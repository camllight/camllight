		   ...
		   error_matching ()
		      ...
		      P_constr (constr2, patt) ->
		        check_same_constr constr constr2;
                        pattern_matching patt (value_nth_field obj 0) ty_arg
      	       	    | _ ->
      	       	        error_matching ())
        ...
        in
          let (ty_res, ty_arg) =
            ...
            (try
             ...
             with Unify ->
               fatal_error "pattern_matching: types should match");
            pattern_matching patt (value_nth_field obj lbl.info.lbl_pos) ty_arg
      in
        (match pattern with
      	 ...
      	 (snd
            (it_list
               (fun (num, list) (pattern, typ) ->
                  (num + 1, (pattern, value_nth_field obj num, typ)::list))
               (0, [])
               val_list)));;
