...
;;; NOTE WELL: comint-mode does *not* do a (kill-all-local-variables).
;;; It is the responsibility of the client to do this. In this sense,
;;; comint-mode does not completely establish a major mode -- just a
;;; large chunk of one. This allows more flexibility to clients
;;; (like ilisp.el).
...
;;; All characters are considered filename constituents by c-m-p-p except:
;;;     !^&*()`'"";{}[]\|<>?
...
;;;
;;; 9/91 Shivers
;;; - Bug fix: changed a set-car to setcar. Lisp, not T.
;;;
;;; 11/91 Shivers
;;; - make-comint binds default-major-mode to 'fundamental-mode
;;;   before creating the process buffer. If your default major mode
;;;   is text-mode, you don't want the text-mode hooks running on
;;;   the process buffer.
;;; - Widened comint-match-partial-pathname's idea of what chars are
;;;   filename constituents. Now, all printing chars are filename
;;;   constituents except: !^&*()`'"";{}[]\|<>?
